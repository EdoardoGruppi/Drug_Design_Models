...
# Define training
def train_epoch(model, data, int_data, optimizer, criterion, model_name='network.pth', output_file='training.txt',
                count=0, t_final=0):
    ...
    for i in range(int(floor(np.shape(data)[0] / batch_size * seq_length))):
        ...
        output_string = f"Loss: {loss.data.item() / seq_length:0.6f} | Delta: {losses[-1] - losses[-2]:+0.4f} | " + \
                        f"Iteration: {i + 1 + count:06d} | Time elapsed: {hours:03d} h {minutes:02d} m {seconds:02d} s"
        print(output_string)
...
def train(model, data, int_data, epochs, model_name, output_file='training.txt', new_session=False):
    ...
    for i in range(epochs):
        # Train
        train_epoch(model, data, int_data, optimizer, criterion, model_name=model_name, output_file=output_file,
