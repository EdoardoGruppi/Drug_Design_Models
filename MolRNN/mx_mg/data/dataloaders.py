...
class MolLoader(DataLoader):
    ...
    def _collate_fn(self, batch):
        ...
        X, A, NX, NA, mol_ids, rep_ids, iw_ids, action_0, actions, last_append_mask, log_p = _build()
        ...
        for i, record_in in enumerate(batch):
            ...
            if i != 0:
                ...
                iw_ids_i += iw_ids[-1] + 1
            X, A, NX, NA, mol_ids, rep_ids, iw_ids, action_0, actions, last_append_mask, \
            log_p = _append([X, A, NX, NA, mol_ids, rep_ids, iw_ids, action_0, actions, last_append_mask, log_p],
                            [X_i, A_i, NX_i, NA_i, mol_ids_i, rep_ids_i, iw_ids_i, action_0_i, actions_i,
                             last_append_mask_i, log_p_i])

        X, A, mol_ids_rep, rep_ids_rep, iw_ids, last_append_mask, NX, NX_rep, action_0, actions, \
        log_p = utils.merge_single(X, A, NX, NA, mol_ids, rep_ids, iw_ids, action_0, actions, last_append_mask, log_p)
        result_out = [X, A, mol_ids_rep, rep_ids_rep, iw_ids, last_append_mask, NX, NX_rep, action_0, actions, log_p]
        return result_out
...
class MolRNNLoader(MolLoader):
    ...
    def from_numpy_to_tensor(record, device_id):
        [X, A, mol_ids_rep, rep_ids_rep, iw_ids, last_append_mask, NX, NX_rep, action_0, actions,
         log_p, graph_to_rnn, rnn_to_graph, NX_cum] = record
