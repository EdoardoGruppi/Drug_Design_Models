...
class MoleculeEnv(gym.Env):
    ...
    def init(self, data_type='zinc', logp_ratio=1, qed_ratio=1, sa_ratio=1, reward_step_total=1, is_normalize=0,
             ...
             conditional='low', max_action=128, min_action=20, force_final=False):
        '''
    ...
    def step(self, action):
        ...
        if self.check_valency():
            if self.mol.GetNumAtoms() + self.mol.GetNumBonds() - self.mol_old.GetNumAtoms() - \
                    self.mol_old.GetNumBonds() > 0:
            ...
            if not self.check_chemical_validity():
                ...
                try:
                    ...
                    if self.reward_type == 'logppen':
                        ...
                        # reward_final += reward_target(final_mol,target=self.reward_target,ratio=0.5,val_max=2,
                        # val_min=-2,func=MolLogP)
                        ...
                        # reward_final += reward_target(final_mol,target=self.reward_target,ratio=0.1,val_max=2,
                        # val_min=-2,func=qed)
                        reward_final += reward_target_qed(final_mol, target=self.reward_target)
                    elif self.reward_type == 'mw_target':
                        # reward_final += reward_target(final_mol,target=self.reward_target,ratio=40,val_max=2,
                        # val_min=-2,func=rdMolDescriptors.CalcExactMolWt)
...
#                                                        'Cl)=C1)=CC=C1NC2=CC=CC=C2C(NC(NC3=C(C(NC4=C(
#                                                        Cl)C=CC=C4)=S)C=CC=C3)=O)=O')), 2) == 4.93

def get_normalized_values():
    fname = '/home/bowen/pycharm_deployment_directory/rl_graph_generation/gym-molecule/gym_molecule/dataset' \
