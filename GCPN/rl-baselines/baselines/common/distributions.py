...
class DiagGaussianPd(Pd):
    ...
    def neglogp(self, x):
        return 0.5 * tf.compat.v1.reduce_sum(tf.square((x - self.mean) / self.std), axis=-1) \
...
class BernoulliPd(Pd):
    ...
    def neglogp(self, x):
        return tf.compat.v1.reduce_sum(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=tf.compat.v1.to_float(x)),
            axis=-1)

    def kl(self, other):
        return tf.compat.v1.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=other.logits, labels=self.ps),
                                       axis=-1) - tf.compat.v1.reduce_sum(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=self.ps), axis=-1)

    def entropy(self):
        return tf.compat.v1.reduce_sum(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.logits, labels=self.ps),
                                       axis=-1)
