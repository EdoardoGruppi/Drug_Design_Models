...
def GCN_batch(adj, node_feature, out_channels, is_act=True, is_normalize=False, name='gcn_simple', aggregate='sum'):
    ...
    with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
        W = tf.compat.v1.get_variable("W", [1, edge_dim, in_channels, out_channels],
                                      initializer=tf.compat.v1.glorot_uniform_initializer())
        b = tf.compat.v1.get_variable("b", [1, edge_dim, 1, out_channels])
        # node_embedding = adj@tf.tile(node_feature,[1,edge_dim,1,1])@tf.tile(W,[batch_size,1,1,1])+b # todo: tf.tile
        #  sum the gradients, may need to change
        node_embedding = adj @ tf.tile(node_feature, [1, edge_dim, 1, 1]) @ tf.tile(W, [batch_size, 1, 1,
                                                                                        1])  # todo: tf.tile sum the
        # gradients, may need to change
        if is_act:
            node_embedding = tf.nn.relu(node_embedding)
        if aggregate == 'sum':
...
def discriminator_net(ob, args, name='d_net'):
    with tf.compat.v1.variable_scope(name, reuse=tf.compat.v1.AUTO_REUSE):
        ob_node = tf.compat.v1.layers.dense(ob['node'], 8, activation=None, use_bias=False,
        ...
        if args.bn == 1:
            emb_node = tf.compat.v1.layers.batch_normalization(emb_node, axis=-1)
        emb_node = GCN_batch(ob['adj'], emb_node, args.emb_size, is_act=False, is_normalize=(args.bn == 0), name='gcn2',
                             aggregate=args.gcn_aggregate)
        if args.bn == 1:
            emb_node = tf.compat.v1.layers.batch_normalization(emb_node, axis=-1)
        # emb_graph = tf.reduce_max(tf.squeeze(emb_node2, axis=1),axis=1)  # B*f
        emb_node = tf.compat.v1.layers.dense(emb_node, args.emb_size, activation=tf.nn.relu, use_bias=False,
                                             name='linear1')
        if args.bn == 1:
            emb_node = tf.compat.v1.layers.batch_normalization(emb_node, axis=-1)

        if args.gate_sum_d == 1:
            emb_node_gate = tf.compat.v1.layers.dense(emb_node, 1, activation=tf.nn.sigmoid, name='gate')
    ...
    def _init(self, ob_space, ac_space, kind, atom_type_num, args):
        ...
        for i in range(args.layer_num_g - 2):
            if args.has_residual == 1:
                ...
                emb_node = tf.concat((GCN_batch(ob['adj'], emb_node, args.emb_size, name='gcn1_' + str(i + 1),
        ...
        ob_len = tf.reduce_sum(
            tf.squeeze(tf.cast(tf.cast(tf.reduce_sum(ob['node'], axis=-1), dtype=tf.bool), dtype=tf.float32), axis=-2),
            axis=-1)  # B
        ob_len_first = ob_len - atom_type_num
        logits_mask = tf.sequence_mask(ob_len, maxlen=tf.shape(ob['node'])[2])  # mask all valid entry
        logits_first_mask = tf.sequence_mask(ob_len_first,
                                             maxlen=tf.shape(ob['node'])[2])  # mask valid entry -3 (rm isolated nodes)
        ...
        ### 2 predict stop
        emb_stop = tf.compat.v1.layers.dense(emb_node, args.emb_size, activation=tf.nn.relu, use_bias=False,
                                             name='linear_stop1')
        if args.bn == 1:
        ...
        # self.logits_stop = tf.concat((tf.reduce_mean(tf.layers.dense(emb_node, 32, activation=tf.nn.relu,
        # name='linear_stop1'),axis=1),tf.reshape(ob_len_first/5,[-1,1])),axis=1)
        ...
        self.logits_first = tf.compat.v1.layers.dense(emb_node, args.emb_size, activation=tf.nn.relu,
                                                      name='linear_select1')
        self.logits_first = tf.squeeze(
            tf.compat.v1.layers.dense(self.logits_first, 1, activation=None, name='linear_select2'), axis=-1)  # B*n
        ...
        self.logits_second = tf.compat.v1.layers.dense(emb_cat, args.emb_size, activation=tf.nn.relu,
                                                       name='logits_second1')
        ...
        self.logits_second_real = tf.compat.v1.layers.dense(emb_cat, args.emb_size, activation=tf.nn.relu,
                                                            name='logits_second1', reuse=True)
        self.logits_second_real = tf.compat.v1.layers.dense(self.logits_second_real, 1, activation=None,
                                                            name='logits_second2', reuse=True)
        # # bilinear
        ...
        self.logits_second_real = tf.where(condition=logits_second_mask_real, x=self.logits_second_real,
        ...
        self.logits_edge = tf.compat.v1.layers.dense(emb_cat, args.emb_size, activation=tf.nn.relu, name='logits_edge1')
        self.logits_edge = tf.compat.v1.layers.dense(self.logits_edge, ob['adj'].get_shape()[1], activation=None,
                                                     name='logits_edge2')
        self.logits_edge = tf.squeeze(self.logits_edge, axis=1)
        # # bilinear
        # self.logits_edge = tf.reshape(bilinear_multi(emb_first,emb_second,out_dim=ob['adj'].get_shape()[1]),[-1,
        # ob['adj'].get_shape()[1]])
        ...
        self.logits_edge_real = tf.compat.v1.layers.dense(emb_cat, args.emb_size, activation=tf.nn.relu,
                                                          name='logits_edge1', reuse=True)
        self.logits_edge_real = tf.compat.v1.layers.dense(self.logits_edge_real, ob['adj'].get_shape()[1],
                                                          activation=None,
                                                          name='logits_edge2', reuse=True)
        self.logits_edge_real = tf.squeeze(self.logits_edge_real, axis=1)
        # # bilinear
        # self.logits_edge_real = tf.reshape(bilinear_multi(emb_first_real, emb_second_real, out_dim=ob[
        # 'adj'].get_shape()[1]),
        ...
        self.pd = self.pdtype(-1).pdfromflat(
            [self.logits_first, self.logits_second_real, self.logits_edge_real, self.logits_stop])
        self.vpred = tf.compat.v1.layers.dense(emb_node, args.emb_size, use_bias=False, activation=tf.nn.relu,
                                               name='value1')
        if args.bn == 1:
        ...
        self.ac = tf.concat((tf.expand_dims(ac_first, axis=1), tf.expand_dims(ac_second, axis=1),
                             tf.expand_dims(ac_edge, axis=1), tf.expand_dims(ac_stop, axis=1)), axis=1)
        ...
        self._act = U.function([stochastic, ob['adj'], ob['node']],
...
if __name__ == "__main__":
    ...
    adj = tf.compat.v1.placeholder(shape=(5, 3, 4, 4), dtype=tf.float32)
    node_feature_np = np.ones((5, 1, 4, 3))
    node_feature = tf.compat.v1.placeholder(shape=(5, 1, 4, 3), dtype=tf.float32)
