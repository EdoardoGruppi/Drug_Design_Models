    ...
    while True:
        ...
        if t > 0 and t % horizon == 0:
            yield {"ob_adj": ob_adjs, "ob_node": ob_nodes, "ob_adj_final": np.array(ob_adjs_final),
                   ...
                   "ep_rets": ep_rets, "ep_lens": ep_lens, "ep_lens_valid": ep_lens_valid, "ep_final_rew": ep_rew_final,
                   "ep_final_rew_stat": ep_rew_final_stat, "ep_rets_env": ep_rets_env, "ep_rets_d_step": ep_rets_d_step,
        ...
        if rew_env > 0:  # if action valid
            ...
            if args.has_d_step == 1:
                if args.gan_type == 'normal' or args.gan_type == 'wgan':
                    ...
                    rew_d_step = args.gan_step_ratio * (
                        max(1 - d_step_func(ob['adj'][np.newaxis, :, :, :], ob['node'][np.newaxis, :, :, :]),
                            -2)) / env.max_atom
            ...
            if args.env == 'molecule':
                with open('molecule_gen/' + args.name_full + '.csv', 'a') as f:
                    ...
                    f.write(str.format(info['smile'], info['reward_valid'], info['reward_qed'], info['reward_sa'],
                                       info['final_stat'], rew_env, rew_d_step, rew_d_final, cur_ep_ret,
                                       info['flag_steric_strain_filter'], info['flag_zinc_molecule_filter'],
                                       info['stop']))
...
def learn(args, env, policy_fn, *,
    ...
    loss_d_step_real = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(logits=step_logit_real, labels=tf.ones_like(step_logit_real) * 0.9))
    ...
    if args.gan_type == 'normal':
        ...
        loss_g_step_gen = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=step_logit_gen, labels=tf.ones_like(step_logit_gen) * 0.9))
    ...
    loss_d_final_real = tf.reduce_mean(
        tf.nn.sigmoid_cross_entropy_with_logits(logits=final_logit_real, labels=tf.ones_like(final_logit_real) * 0.9))
    ...
    if args.gan_type == 'normal':
        ...
        loss_g_final_gen = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(logits=final_logit_gen, labels=tf.ones_like(final_logit_gen) * 0.9))
    elif args.gan_type == 'wgan':
        loss_d_final, _, _ = discriminator(ob_real, ob_gen, args, name='d_final')
    ...
    var_list_pi_stop = [var for var in var_list_pi if
                        ('emb' in var.name) or ('gcn' in var.name) or ('stop' in var.name)]
    ...
    lossandgrad_ppo = U.function([ob['adj'], ob['node'], ac, pi.ac_real, oldpi.ac_real, atarg, ret, lrmult],
                                 losses + [U.flatgrad(total_loss, var_list_pi)])
    lossandgrad_expert = U.function([ob['adj'], ob['node'], ac, pi.ac_real],
                                    [loss_expert, U.flatgrad(loss_expert, var_list_pi)])
    lossandgrad_expert_stop = U.function([ob['adj'], ob['node'], ac, pi.ac_real],
                                         [loss_expert, U.flatgrad(loss_expert, var_list_pi_stop)])
    lossandgrad_d_step = U.function([ob_real['adj'], ob_real['node'], ob_gen['adj'], ob_gen['node']],
    ...
    while True:
        ...
        for i_optim in range(optim_epochs):
            ...
            if iters_so_far >= args.expert_start and iters_so_far <= args.expert_end + pretrain_shift:
                ...
                # loss_expert_stop, g_expert_stop = lossandgrad_expert_stop(ob_expert['adj'], ob_expert['node'],
                # ac_expert,ac_expert)
            ...
            if iters_so_far >= args.rl_start and iters_so_far <= args.rl_end:
                ...
                # ppo
                # if args.has_ppo==1:
                if iters_so_far >= args.rl_start + pretrain_shift:  # start generator after discriminator trained a
                    # well..
                ...
                if args.has_d_final == 1 and i_optim >= optim_epochs // 4 * 3:
                    ...
                    loss_d_final, g_d_final = lossandgrad_d_final(ob_expert["adj"], ob_expert["node"], seg_final_adj,
                                                                  seg_final_node)
                    # loss_d_final, g_d_final = lossandgrad_d_final(ob_expert["adj"], ob_expert["node"], ob_adjs,
                    # ob_nodes)
        ...
        lens, lens_valid, rews, rews_env, rews_d_step, rews_d_final, rews_final, rews_final_stat = map(flatten_lists,
                                                                                                       zip(
                                                                                                           *listoflrpairs))
