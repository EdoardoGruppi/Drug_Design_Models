...
if __name__ == "__main__":
    ...
    parser.add_argument('--folder', type=str, default='train_processed')
    ...
    if args.mode == 'pair':
        ...
        for split_id in range(num_splits):
            st = split_id * le
            print(f'Processing split number {split_id}, i.e. from {st} to {st + le} of the {len(batches)} total batches.')
            # sub_data = all_data[st: st + le]
            sub_data = process_map(func, batches[st: st + le])
